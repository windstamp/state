#!/usr/bin/env python3
"""
Analyze profiling results from State training.

Usage:
    python analyze_state_profile.py --profile_dir ./profiling_logs
    python analyze_state_profile.py --profile_dir ./profiling_logs --model_checkpoint ~/state/test/checkpoints/final.ckpt
"""
...
def main():
    parser = argparse.ArgumentParser(description='Analyze State model profiling results')
    ...
    print(f"\n{'='*80}")
    print(f"Analyzing State Model Profiling Results")
    print(f"{'='*80}\n")
...
def analyze_model_summary_from_checkpoint(checkpoint):
    ...
    important_keys = [
        'hidden_dim', 'cell_set_len', 'n_encoder_layers', 'n_decoder_layers',
